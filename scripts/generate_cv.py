...
Freelance Python / Full Stack Developer
Remote | Nov 2023 - Present
- Built automation tools, REST APIs, and scraping pipelines in Python (Flask/FastAPI).
- Shipped React frontends integrated with PostgreSQL backends; deployed on Vercel and Render.
- Integrated third-party APIs: Deezer, Groq, Cloudinary; JWT auth and SQLAlchemy data models.

Full Stack Developer - Capstone Project
4Geeks Academy | Jun 2025 - Present
...
React | TypeScript | FastAPI | PostgreSQL | Vercel | Deezer API | JWT/RBAC
...
- Decomposes complex goals into 5 actionable steps; task history, PDF export, admin auth.
...
Backend: FastAPI | Flask | SQLAlchemy | Alembic | Pydantic | JWT/RBAC | REST APIs | pytest
Databases: PostgreSQL | Neon | SQLite
DevOps: Docker | GitHub Actions | Vercel | Render | Linux | CI/CD
APIs: Deezer API | Groq API | Cloudinary"""

SKILLS_BACKEND = """SKILLS
Languages: Python | JavaScript | TypeScript | Bash
Backend: FastAPI | Flask | SQLAlchemy | Alembic | Pydantic | JWT | REST APIs | pytest
Databases: PostgreSQL | Neon | SQLite
DevOps: Docker | GitHub Actions | Vercel | Render | Linux | CI/CD
...
CERTS = """CERTIFICATIONS (secondary)
...
def write_pdf(path: Path, about: str, projects: str, skills: str):
    ...
    pdf.multi_cell(w, 5, "Junior+ Full Stack Developer | Python (FastAPI/Flask) + React")
    ...
    pdf.multi_cell(w, 4.5, "Portfolio: portofolio-mu-lac.vercel.app | Open to remote full stack roles")
    pdf.ln(3)
...
def main():
    ...
    print("Generated:", ", ".join(p.name for p in PUBLIC.glob("*.pdf")))
